import MaxWidthWrapper from "@/components/custom/MaxWidthWrapper";
import { Button } from "@/components/ui/button";

const projects = [
  {
    id: 1,
    title: "Advanced Medical Center",
    description: "Hospital Construction",
    location: "Dhanmondi, Dhaka",
    imageUrl: "/adhy-savala-zbpgmGe27p8-unsplash.jpg",
  },
  {
    id: 2,
    title: "Corporate Office Solutions",
    description: "Office Equipment Supply",
    location: "Gulshan, Dhaka",
    imageUrl: "/benjamin-child-0sT9YhNgSEs-unsplash.jpg",
  },
  {
    id: 3,
    title: "Luxury Private Clinic",
    description: "Hospital Construction",
    location: "Banani, Dhaka",
    imageUrl: "/graham-ruttan-aMNLYoT2z_I-unsplash.jpg",
  },
  {
    id: 4,
    title: "Commercial Medical Office",
    description: "Medical Equipment Supply",
    location: "Uttara, Dhaka",
    imageUrl: "/national-cancer-institute-oCLuFi9GYNA-unsplash.jpg",
  },
];

const ProjectShowcase = () => {
  return (
    <>
      <MaxWidthWrapper>
        <h1 className="text-3xl md:text-5xl font-semibold text-si-primary mb-5">
          Featured Projects
        </h1>
      </MaxWidthWrapper>

      <div className="relative min-h-screen w-full">
        <div
          className="absolute inset-0 bg-cover bg-center"
          style={{
            backgroundImage: `url(/solen-feyissa-jGmBZypoFPc-unsplash.jpg)`,
          }}
        >
          <MaxWidthWrapper className="w-full h-full">
            <section className="grid grid-cols-2 lg:gap-0 lg:grid-cols-4 h-full w-full p-[5%]">
              {projects.map((project) => (
                <div key={project.id} className="bg-green-500 h-full w-full">
                  <div className="relative w-full h-full">
                    <div
                      className="absolute inset-0 bg-cover bg-center"
                      style={{
                        backgroundImage: `url(${project.imageUrl})`,
                      }}
                    >
                      {/* Black overlay */}
                      <div className="absolute inset-0 bg-black opacity-40"></div>
                      <section className="relative p-6 pt-10 text-muted">
                        <div>
                          <p className="text-base">{project.description}</p>
                          <h4 className="text-xl font-medium">
                            {project.title}
                          </h4>
                        </div>
                        <h6 className="text-sm mt-3 mb-10">
                          {project.location}
                        </h6>
                        <Button>View More</Button>
                      </section>
                    </div>
                  </div>
                </div>
              ))}
            </section>
          </MaxWidthWrapper>
        </div>
      </div>
    </>
  );
};

export default ProjectShowcase;
